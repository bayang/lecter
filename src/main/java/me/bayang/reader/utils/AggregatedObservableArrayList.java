...
public class AggregatedObservableArrayList<T> {
    ...
    /**
     * This class aggregates several other Observed Lists (sublists), observes
     * changes on those sublists and applies those same changes to the
    ...
    final protected ObservableList<T> aggregatedList = FXCollections
            .observableArrayList();
    ...
    /**
     * The Aggregated Observable List. This list is unmodifiable, because
     * sorting this list would mess up the entire bookkeeping we do here.
     *
    ...
    public void appendList(ObservableList<T> list) {
        ...
        lists.add(list);
        final InternalListModificationListener listener = new InternalListModificationListener(
                list);
        list.addListener(listener);
        // System.out.println("list = " + list + " puttingInMap=" +
        // list.hashCode());
        sizes.add(list.size());
        aggregatedList.addAll(list);
        listeners.add(listener);
        assert lists.size() == sizes.size()
                && lists.size() == listeners.size() : "lists.size="
                        + lists.size() + " not equal to sizes.size="
                        + sizes.size() + " or not equal to listeners.size="
                        + listeners.size();
    }
    ...
    public void removeList(ObservableList<T> list) {
        ...
        if (index < 0) {
            throw new IllegalArgumentException(
                    "Cannot remove a list that is not contained: " + list
                            + " lists=" + lists);
        }
        final int startIndex = getStartIndex(list);
        final int endIndex = getEndIndex(list, startIndex);
        // we want to find the start index of this list inside the aggregated
        // List. End index will be start + size - 1.
        lists.remove(list);
        sizes.remove(index);
        final InternalListModificationListener listener = listeners
                .remove(index);
        list.removeListener(listener);
        aggregatedList.remove(startIndex, endIndex + 1); // end + 1 because end
                                                         // is exclusive
        assert lists.size() == sizes.size()
                && lists.size() == listeners.size() : "lists.size="
                        + lists.size() + " not equal to sizes.size="
    ...
    /**
     * Get the start index of this list inside the aggregated List. This is a
     * private function. we can safely asume, that the list is in the map.
     *
     * @param list
    ...
    private int getStartIndex(ObservableList<T> list) {
        ...
        assert lists.size() == sizes.size() : "lists.size=" + lists.size()
                + " not equal to sizes.size=" + sizes.size();
        final int listIndex = lists.indexOf(list);
        for (int i = 0; i < listIndex; i++) {
            final Integer size = sizes.get(i);
            startIndex += size;
            // System.out.println(" startIndex = " + startIndex + " added=" +
            // size);
        }
    ...
    /**
     * Get the end index of this list inside the aggregated List. This is a
     * private function. we can safely asume, that the list is in the map.
     *
     * @param list
     *            the list in question
     * @param startIndex
     *            the start of the list (retrieve with
     *            {@link #getStartIndex(ObservableList)}
    ...
    private class InternalListModificationListener
        ...
        public void onChanged(Change<? extends T> change) {
            ...
            // System.out.println("onChanged for list=" + list + " aggregate=" +
            // aggregatedList);
            while (change.next()) {
                final int from = change.getFrom();
                final int to = change.getTo();
                // System.out.println(" startIndex=" + startIndex + " from=" +
                // from + " to=" + to);
                if (change.wasPermutated()) {
                    final ArrayList<T> copy = new ArrayList<>(aggregatedList
                            .subList(startIndex + from, startIndex + to));
                    // System.out.println(" permutating sublist=" + copy);
                    for (int oldIndex = from; oldIndex < to; oldIndex++) {
                        int newIndex = change.getPermutation(oldIndex);
                        copy.set(newIndex - from,
                                aggregatedList.get(startIndex + oldIndex));
                    }
                    // System.out.println(" permutating done sublist=" + copy);
                    aggregatedList.subList(startIndex + from, startIndex + to)
                            .clear();
                    ...
                    if (change.wasRemoved()) {
                        ...
                        aggregatedList.remove(startIndex + from,
                                startIndex + from + removed.size());
                    }
                    if (change.wasAdded()) {
